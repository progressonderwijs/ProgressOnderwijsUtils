...
namespace ProgressOnderwijsUtils.Tests
{
    ...
    public sealed class HtmlTests
    {
        ...
        {
            var example = _div._class("A", "B", null, "D").AsFragment().ToStringWithDoctype();
        ...
        {
            var example = _div.Attributes(Enumerable.Range(1,5).Select(i => "data-" + i.ToString()).Select(s=> new HtmlAttribute(s,"x"))).AsFragment().ToStringWithDoctype();
