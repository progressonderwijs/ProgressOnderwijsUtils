...
{
    static readonly ColumnSort ziggyA = new("ziggy", SortDirection.Asc);
    static readonly ColumnSort ziggyD = new("ziggy", SortDirection.Desc);
    static readonly ColumnSort abcA = new("abc", SortDirection.Asc);
    static readonly ColumnSort abcD = new("abc", SortDirection.Desc);
    static readonly ColumnSort monsterA = new("monster", SortDirection.Asc);
    ...
    public void AssumeThenByOk()
    {
        ...
        PAssert.That(() => new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { acolD, })) == new OrderByColumns(new[] { ziggyA, abcA, }));
        PAssert.That(() => new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { abcA, acolD, })) == new OrderByColumns(new[] { ziggyA, }));
        PAssert.That(() => new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { ziggyA, abcA, acolD, })) == OrderByColumns.Empty);
        PAssert.That(
            () =>
                new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { acolD, ziggyA, abcA, }))
                == new OrderByColumns(new[] { ziggyA, abcA, })
        );
        PAssert.That(
            () =>
                new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { abcA, ziggyA, acolD, }))
                == new OrderByColumns(new[] { ziggyA, abcA, acolD, })
        );
        PAssert.That(
            () =>
                new OrderByColumns(new[] { ziggyA, abcA, acolD, }).AssumeThenBy(new(new[] { ziggyA, abcA, acolA, }))
                == new OrderByColumns(new[] { ziggyA, abcA, acolD, })
        );
    }
}
