...
public sealed class SortedArraySetTest
{
    [Fact]
    public void TheEmptySetIsEmpty()
    {
        var set = SortedArraySet.FromValues(Array.Empty<int>());
    ...
    {
        var set = SortedArraySet.FromValues(new[] { 1, 2, 3, });
    ...
    {
        var set = SortedArraySet.FromValues(new[] { 1, 2, 3, 3, 2, 1, });
    ...
    {
        var set = SortedArraySet.FromValues(new[] { 1, 2, 4, 8, 16, 32, });
    ...
    {
        var set = SortedArraySet.FromValues(Enumerable.Range(0, 1000).Select(i => i * 2).Reverse());
    ...
    {
        var superset = SortedArraySet.FromValues([22462, 113372, 115148, 197410,]);
        PAssert.That(() => !SortedArraySet.FromValues(new[] { 197665, }).IsSubsetOf(superset));
        PAssert.That(() => SortedArraySet.FromValues(new[] { 115148, }).IsSubsetOf(superset));
        PAssert.That(() => SortedArraySet.Empty<int>().IsSubsetOf(superset));
    }

    [Fact]
    public void IsSubSet_large_example()
    {
        var superset = SortedArraySet.FromValues(Enumerable.Range(0, 1000).Select(i => i * i));

        PAssert.That(() => !SortedArraySet.FromValues(new[] { 36, 49, 50, 64, }).IsSubsetOf(superset));
        PAssert.That(() => SortedArraySet.FromValues(new[] { 36, 49, 64, 100, 400, 256, }).IsSubsetOf(superset));
        PAssert.That(() => SortedArraySet.Empty<int>().IsSubsetOf(superset));
    }
    ...
    public void IsSubSet_manyTests()
    {
        for (var size = 0; size < 100; size++) {
            for (var largerSize = size; largerSize < size * 20 + 100; largerSize += 1 + (largerSize >> 2)) {
                ...
                var smallContainedSet = SortedArraySet.FromValues(largeSetValues[..size]);
                var smallNonContainedSet = SortedArraySet.FromValues(largeSetValues[..size].Append(r.Next(100_000) * 2 + 1));
                var largeSet = SortedArraySet.FromValues(largeSetValues);
