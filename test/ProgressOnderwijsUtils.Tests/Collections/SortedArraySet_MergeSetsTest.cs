...
public sealed class SortedArraySet_MergeSetsTest
{
    [Fact]
    public void MergeOfTwoEmptySetsIsEmpty()
    {
        var mergeResult = new[] { SortedArraySet.Empty<int>(), SortedArraySet.Empty<int>(), }.MergeSets();
    ...
    {
        var mergeResult = Array.Empty<SortedArraySet<int>>().MergeSets();
        PAssert.That(() => mergeResult.ValuesInOrder.Length == 0);
    }
    ...
    {
        var set = SortedArraySet.FromValues([3, 5, 2, 9, 9,]);
    ...
    {
        var setA = SortedArraySet.FromValues([3, 5, 2, 9, 9,]);
        var setB = SortedArraySet.FromValues([2, 8, 7, 6, 2,]);
        var setC = SortedArraySet.FromValues([1, 3, 4,]);
        var setExpected = SortedArraySet.FromValues(Enumerable.Range(1, 9));
    ...
    {
        var setA = SortedArraySet.FromValues([3, 5,]);
        var setB = SortedArraySet.FromValues([2, 8, 7, 6, 2, 2, 9, 9, 10, 4, 1,]);
        var setExpected = SortedArraySet.FromValues(Enumerable.Range(1, 10));
    ...
    public void ALargeMergeWorks()
    {
        var sets = Enumerable.Range(1, 100)
            .Select(n => SortedArraySet.FromValues(Enumerable.Range(1, 1000).Select(i => i * n % 2345)));
        var setExpected = SortedArraySet.FromValues(Enumerable.Range(0, 2345));
    ...
    public void TwoWayMergeRemovesDuplicatedItems()
    {
        var setA = SortedArraySet.FromValues([3, 5, 2, 9, 9,]);
        var setB = SortedArraySet.FromValues([2, 8, 7, 6, 2,]);
        var output = SortedArraySet<int>.Algorithms.Merge_RemovingDuplicates(setA.ValuesInOrder, setB.ValuesInOrder, Comparer<int>.Default);
    ...
    public void Exhaustive5bitTwoWayMergeCheck()
    {
        ...
        foreach (var a in sets) {
            foreach (var b in sets) {
                ...
                PAssert.That(() => SortedArraySet<int>.Algorithms.Merge_RemovingDuplicates(setA, setB, Comparer<int>.Default).SequenceEqual(expected));
            }
        }
    }
}
