...
public sealed class AsSqlInExpressionTest : TransactedLocalConnection
{
    ...
    public void In_helper_works_on_actual_query()
    {
        ...
        PAssert.That(() => SQL($"select t.x from Tin t where t.x in {Array.Empty<int>().AsSqlInExpression()}").ReadPlain<int?>(Connection).None());
        PAssert.That(() => SQL($"select t.y from Tin t where t.y in {Array.Empty<int>().AsSqlInExpression()}").ReadPlain<int?>(Connection).None());

        PAssert.That(() => SQL($"select t.x from Tin t where t.x in {new[] { 3, }.AsSqlInExpression()}").ReadPlain<int?>(Connection).Single() == 3);
        PAssert.That(() => SQL($"select t.x from Tin t where t.x in {new[] { 999, }.AsSqlInExpression()}").ReadPlain<int?>(Connection).None());

        PAssert.That(() => SQL($"select t.y from Tin t where t.y in {new[] { 7, 999, }.AsSqlInExpression()}").ReadPlain<int?>(Connection).Single() == 7);
    }
}
