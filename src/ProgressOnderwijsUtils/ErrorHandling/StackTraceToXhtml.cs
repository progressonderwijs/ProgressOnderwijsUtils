	...
	public static class StackTraceToXhtml
	{
		...
		static IEnumerable<XNode> decompose(string trace)
		{
			...
			foreach (Match m in stackLine.Matches(trace))
			{
				...
				var fileLocEl = !m.Groups["file"].Success ? null :
					new XElement("span", new XAttribute("class", "stacktrace-src"),
						" in ", m.Groups["path"].Value,
						new XElement("span", new XAttribute("class", "stacktrace-file"), m.Groups["file"].Value),
						":line ", new XElement("span", new XAttribute("class", "stacktrace-line"), m.Groups["line"].Value)
						);

				yield return new XElement("tr", new XAttribute("class", "stacktrace-frame"),
					new XElement("td", new XAttribute("class", "stacktrace-class"),
					...
					new XElement("td", new XAttribute("class", "stacktrace-method"),
