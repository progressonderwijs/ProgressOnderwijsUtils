...
namespace ProgressOnderwijsUtils.Conext
{
	...
	public static class MetaDataFactory
	{
		...
		const string IDP_CERTIFICATE = "conext.cer";
		const string IDP_PROVIDER = "https://engine.surfconext.nl/authentication/proxy/idps-metadata/key:20140505";
		const string IDP_PROVIDER_WAYF = "https://engine.surfconext.nl/authentication/idp/metadata/key:20140505";
		const string IDP_ENTITY = "https://engine.surfconext.nl/authentication/proxy/idps-metadata/key:20140505";
		const string IDP_ENTITY_WAYF = "https://engine.surfconext.nl/authentication/idp/metadata";
		const string IDP_ENTITY_RUG = "https://signon.rug.nl/nidp/saml2/metadata";
		const string IDP_ENTITY_FONTYS = "http://adfs2.fontys.nl/adfs/services/trust";
		const string IDP_ENTITY_VU = "https://surf-sso.ubvu.vu.nl/simplesaml/saml2/idp/metadata.php";
		const string IDP_ENTITY_UVA = "https://secure.uva.nl/cas";
		const string IDP_ENTITY_STENDEN = "http://adfs.stenden.com/adfs/services/trust";
		const string SP_CERTIFICATE = "surff.pfx";
		const string SP_PROVIDER = "https://progressnet.nl/saml20/";
		const string SP_ENTITY_P3W = "http://progresswww.nl";
		const string SP_ENTITY_P3W_TEST = "http://progresswww.nl/test";
		const string SP_ENTITY_PNET = "http://progressnet.nl";
		const string SP_ENTITY_PNET_TEST = "http://test.progressnet.nl";
		const string SP_ENTITY_PNET_ONTWIKKEL = "http://ontwikkel.progressnet.nl";
		const string SP_ENTITY_STUDENT = "http://student.progressnet.nl";
		const string SP_ENTITY_STUDENT_TEST = "http://teststudent.progressnet.nl";
		const string SP_ENTITY_STUDENT_ONTWIKKEL = "http://ontwikkelstudent.progressnet.nl";
		const string SP_ENTITY_STUDENT_OAUTH = "http://student.progressnet.nl/oauth";
		const string SP_ENTITY_STUDENT_TEST_OAUTH = "http://teststudent.progressnet.nl/oauth";
		const string SP_ENTITY_STUDENT_ONTWIKKEL_OAUTH = "http://ontwikkelstudent.progressnet.nl/oauth";
		static readonly ConcurrentDictionary<string, X509Certificate2> CERTIFICATES = new ConcurrentDictionary<string, X509Certificate2>();
		static readonly IDictionary<string, Saml20MetaData> INSTANCES = new Dictionary<string, Saml20MetaData>();
		static readonly XNamespace NS_XSD = "http://www.w3.org/2001/XMLSchema-instance";
		static readonly XNamespace NS_DS = "http://www.w3.org/2000/09/xmldsig#";
		static readonly XNamespace NS_MD = "urn:oasis:names:tc:SAML:2.0:metadata";

		static readonly IDictionary<ServiceProvider, IDictionary<DatabaseVersion, ServiceProviderConfig>> SERVICE_PROVIDERS = new Dictionary<ServiceProvider, IDictionary<DatabaseVersion, ServiceProviderConfig>>
		{
			{
		...
		static readonly IDictionary<IdentityProvider, IdentityProviderConfig> IDENTITY_PROVIDERS = new Dictionary<IdentityProvider, IdentityProviderConfig>
		{
			{
		...
		static readonly IDictionary<IdentityProvider, IDictionary<ServiceProvider, IDictionary<DatabaseVersion, IDictionary<Entity, string>>>> ENTITIES =
		...
		static readonly IDictionary<RootOrganisatie, Entity> INSTELLINGEN = new Dictionary<RootOrganisatie, Entity>
		{
		...
		static Saml20MetaData GetIdPProvider(string url, string sp, X509Certificate2 cer)
		{
		...
		static XDocument Generate(Guid id, X509Certificate2 cer)
		{
		...
		static XElement GenerateEntity(X509Certificate2 cer, string entity, string post, string redirect = null, string slo = null)
		{
		...
		static XElement GenerateOrganization()
		{
		...
		static XElement GenerateTechnicalContact()
		{
			return GenerateContact("technical", "Fons", "Dijkstra", "f.j.dijkstra@rug.nl", "+31(0)50 363 6671");
		}

		static XElement GenerateAdministrativeContact()
		{
			return GenerateContact("administrative", "Jan", "Batteram", "j.batteram@rug.nl", "+31(0)50 363 2000");
		}

		static XElement GenerateContact(string type, string givenName, string surName, string email, string phone)
		{
		...
		static XmlDocument Sign(XDocument md, X509Certificate2 cer)
		{
