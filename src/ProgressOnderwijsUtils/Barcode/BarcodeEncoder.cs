...
namespace ProgressOnderwijsUtils.Barcode
{
	public static class BarcodeEncoder
	{
		...
		public static BarcodeWaarde Decode(string value)
		{
			...
			if (value.Length != EncodedLength) throw new BarcodeException("Fout BarcodeWaarde-lengte");
		...
		private static string Encode(BarcodeWaarde barcodeWaarde)
		{
			if (barcodeWaarde.Type == BarcodeType.Onbekend) throw new BarcodeException(Enum.GetName(typeof(BarcodeType), barcodeWaarde.Type));
			if (barcodeWaarde.Value < 0) throw new BarcodeException(barcodeWaarde.Value.ToString(CultureInfo.InvariantCulture));
		...
		static int Checksum(int value)
		{
			return value % BarcodeEncodingValue.AANTAL_KARAKTERS;
		}
	}
}
