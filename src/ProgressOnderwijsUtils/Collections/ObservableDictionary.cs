...
{
	public enum DictionaryChange { Add, Set, Remove, Clear }
	...
	public static class ObservableDictionary
	{
		...
		public static IDictionary<TKey, TValue> ObserveRealChanges<TKey, TValue>(this IDictionary<TKey, TValue> dict, Action onchange) where TValue : IEquatable<TValue>
		{
			if (dict == null) throw new ArgumentNullException("dict");
			if (onchange == null) throw new ArgumentNullException("onchange");
			return new ObservableDictionaryImpl<TKey, TValue>(dict, (change, key, newval, oldval) =>
			{
		...
		sealed class ObservableDictionaryImpl<TKey, TValue> : IDictionary<TKey, TValue>
		{
			...
			internal ObservableDictionaryImpl(IDictionary<TKey, TValue> implementationToWrap, Action<DictionaryChange, TKey, TValue, TValue> onchange) { impl = implementationToWrap; changed = onchange; }

			public void Add(TKey key, TValue value) { impl.Add(key, value); changed(DictionaryChange.Add, key, value, default(TValue)); }
			...
			public void Add(KeyValuePair<TKey, TValue> item) { impl.Add(item); changed(DictionaryChange.Add, item.Key, item.Value, default(TValue)); }
			public void Clear() { impl.Clear(); changed(DictionaryChange.Clear, default(TKey), default(TValue), default(TValue)); }
