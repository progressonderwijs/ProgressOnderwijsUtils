...
namespace ProgressOnderwijsUtils.Log4Net
{
	public static class ILogExtensions
	{
		...
		{
			if (log.Value.IsDebugEnabled) log.Value.Debug(msg());
		}
        ...
        {
            if (log.Value.IsInfoEnabled) log.Value.Info(msg());
        }
		...
		{
			if (log.Value.IsWarnEnabled) log.Value.Warn(msg());
		}
		...
		{
			if (log.Value.IsErrorEnabled) log.Value.Error(msg());
		}
